...
<vertical>
    ...
    <vertical>
        ...
        {/* <button id="bt" layout_gravity="center" color="#ff5500" text="开始运行"/> */}
...
function MainFunction() {
    ...
    toast("已设置抢购时间为:"+xHour+"时"+xMinute+"分"+xSecond+ "秒"+xMillis+"毫秒")
    sleep(1000)
    ...
    while(true){
        ...
        if(xHour == hour && xMinute == minute && xSecond == second && xMillis <= millis){
            ...
            var 结算=text("结算").findOne();press(结算.bounds().centerX(),结算.bounds().centerY(),1)
            toast("点击结算")
            var 提交订单=text("提交订单").findOne();press(提交订单.bounds().centerX(),提交订单.bounds().centerY(),1)
            break;
        }
    }
